...
public class Network {
    ...
    public boolean addUser(String name) {
        ...
        User New = new User(name);
        this.users[userCount] = New;
        userCount++;
        return true;
    }
    ...
    public String recommendWhoToFollow(String name) {
        ...
        User mostRecUser = null;
        int maxMutual = -1;
        for (int i = 0; i < userCount; i++) {
            User temp = users[i];
            if (!temp.getName().equals(name) && !current.follows(temp.getName())) {
                int mutual = current.countMutual(temp);
                if (mutual > maxMutual) {
                    maxMutual = mutual;
                    mostRecUser = temp;
                }
    ...
    public String mostPopularUser() {
        ...
        for (int i = 0; i < userCount; i++) {
            ...
            int count = followeeCount(currUser.getName());
            if (count > max) {
                max = count;
                mostPopular = currUser;
